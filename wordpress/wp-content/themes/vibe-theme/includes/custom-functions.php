 ...
 */
add_action( 'wp_enqueue_scripts', 'vibe_deregister_frontend_scripts', 20 );
function vibe_deregister_frontend_scripts() {
    if ( ! is_admin() ) {
...
add_action( 'init', 'vibe_disable_frontend_features', 10 );
function vibe_disable_frontend_features() {
    if ( is_admin() ) {
        return;
    }
 ...
 */
function vibe_prepare_macros( $str ) {
	return str_replace(
		[
			'((',
			'))',
...
function vibe_allow_html_in_title( $title ) {
	$title = vibe_prepare_macros( $title );
	return $title;
}

add_filter( 'the_title', 'vibe_allow_html_in_title', 10, 2 );
add_filter( 'the_content', 'vibe_allow_html_in_title', 10, 2 );

function vibe_filter_title_parts( $title ) {
        $title['title'] = str_replace( '((', '', $title['title'] );
        $title['title'] = str_replace( '))', '', $title['title'] );
    return $title;
}

add_filter( 'document_title_parts', 'vibe_filter_title_parts' );
 ...
 */
add_action( 'template_redirect', 'vibe_setup_custom_header_footer', 1 );
function vibe_setup_custom_header_footer() {
    if ( is_admin() ) {
        return;
    }
    ...
    if ( $headerSwitch ) {
        ...
        add_action( 'generate_header', 'vibe_render_custom_header', 10 );
    }
    ...
    if ( $footerSwitch ) {
        ...
        add_action( 'generate_footer', 'vibe_render_custom_footer', 10 );
    }
}
 ...
 */
function vibe_render_custom_header() {
 ...
 */
function vibe_render_custom_footer() {
