 ...
 */
add_action( 'wp_enqueue_scripts', 'nexus_deregister_frontend_scripts', 20 );
function nexus_deregister_frontend_scripts() {
    if ( ! is_admin() ) {
...
add_action( 'init', 'nexus_disable_frontend_features', 10 );
function nexus_disable_frontend_features() {
    if ( is_admin() ) {
        return;
    }
 ...
 */
function nexus_prepare_macros( $str ) {
	return str_replace(
		[
			'((',
			'))',
...
function nexus_allow_html_in_title( $title ) {
	$title = nexus_prepare_macros( $title );
	return $title;
}

add_filter( 'the_title', 'nexus_allow_html_in_title', 10, 2 );
add_filter( 'the_content', 'nexus_allow_html_in_title', 10, 2 );

function nexus_filter_title_parts( $title ) {
        $title['title'] = str_replace( '((', '', $title['title'] );
        $title['title'] = str_replace( '))', '', $title['title'] );
    return $title;
}

add_filter( 'document_title_parts', 'nexus_filter_title_parts' );
 ...
 */
add_action( 'template_redirect', 'nexus_setup_custom_header_footer', 1 );
function nexus_setup_custom_header_footer() {
    if ( is_admin() ) {
        return;
    }
    ...
    if ( $headerSwitch ) {
        ...
        add_action( 'generate_header', 'nexus_render_custom_header', 10 );
    }
    ...
    if ( $footerSwitch ) {
        ...
        add_action( 'generate_footer', 'nexus_render_custom_footer', 10 );
    }
}
 ...
 */
function nexus_render_custom_header() {
 ...
 */
function nexus_render_custom_footer() {
